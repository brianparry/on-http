...
function workflowApiServiceFactory(
    ...
    //todo move to on-core and use mongo query.
    WorkflowApiService.prototype.isRunning = function(workflow) {
        return Promise.resolve(workflow)
        .then(function(workflow) {
            return _(workflow.tasks).every(function(task) {
    ...
    WorkflowApiService.prototye.deleteTaskGraph = function(graphId) {
        var self = this;

        return waterline.graphobjects.needByIdentifier(id)
        .then(function(workflow) {
            if (!self.isRunning(workflow)) {
                //todo Do I need to delete tasks too?
                taskGraphStore.deleteGraph(id);            
            }
        })
    };
