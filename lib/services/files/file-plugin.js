...
function factory(Promise, Errors, waterline, uuid, assert,  _, fs) {
    ...
    function _onMetadata(stream, query, filename, id, method, metadata) {
        var self = this;
        var methods;
        method = method || 'put';
    ...
    FileManager.prototype.post = function(query) {
        var id = uuid('v4');
        var filename = path.basename(query) + '_' + id;
        var wrStream = this.createWriteStream('unresolved_'+filename);

        wrStream.on('metadata', _onMetadata.bind(
            this, wrStream, query, filename, id, 'post'
        ));
    ...
    FileManager.prototype.put = function(query) {
        ...
        wrStream.on('metadata', _onMetadata.bind(
            this, wrStream, query, filename, id, 'put'
        ));
